...
export const MailPromoModal: React.FC<MailPromoModalProps> = ({
  ...
  const handleSendCustomMail = async (e: React.FormEvent) => {
    ...
    if (promoImage) {
      const base64Data = promoImage.split(',')[1];
      attachments.push({
        name: promoImageName || 'imagen.png',
        data: base64Data
      });
    }
    if (pdfFile) {
      ...
      attachments.push({
        name: pdfFileName || 'documento.pdf',
        data: base64Data
      });
    }
  ...
  const handlePdfChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isPdf) {
      alert('Por favor, seleccione únicamente archivos PDF.');
      return;
    }
    const maxSizeBytes = 10 * 1024 * 1024; // 10MB
    if (file.size > maxSizeBytes) {
      alert(`El archivo PDF supera el límite de 10 MB (su archivo pesa ${(file.size / (1024 * 1024)).toFixed(2)} MB).`);
      return;
    }
    setPdfFileName(`${file.name} (${(file.size / (1024 * 1024)).toFixed(2)} MB)`);
    const reader = new FileReader();
    reader.onloadend = () => {
    ...
    <div className="fixed inset-0 bg-brand-gray-dark/75 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fade-in font-sans">
      <div className="bg-white rounded-3xl border border-slate-100 shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <form onSubmit={handleSendCustomMail} className="p-6 space-y-4 overflow-y-auto flex-1">
          ...
          <div className="p-4 bg-slate-50 border border-slate-200/50 rounded-2xl space-y-2">
            ...
            <div className="flex items-center gap-3">
              <label 
                ...
                className="bg-brand-gray-dark hover:bg-gray-800 text-white font-extrabold text-[10px] px-4 py-2 rounded-xl cursor-pointer uppercase tracking-wider transition"
              >
                📁 Cargar Imagen
              </label>
              <input 
          ...
          <div className="p-4 bg-slate-50 border border-slate-200/50 rounded-2xl space-y-2">
            <label className="text-[9px] font-black text-gray-500 uppercase tracking-wider block">Adjuntar Documento PDF (Opcional)</label>
            <div className="flex items-center gap-3">
              <label 
              ...
              <input 
                ...
                className="hidden" 
              />
              <span className="text-[10px] font-mono font-bold text-slate-500 truncate max-w-[200px]">{pdfFileName || "Ningún archivo seleccionado"}</span>
            </div>

            {pdfFile && (
              <div className="mt-2 flex items-center justify-between p-2.5 bg-white border border-slate-200 rounded-xl">
                <div className="flex items-center gap-2 text-3xs font-mono font-bold text-red-600 truncate max-w-[80%]">
                  <span>📄</span>
                ...
                <button
                  ...
                  className="bg-red-600 hover:bg-red-700 text-white p-1 rounded-full text-[10px] w-5 h-5 flex items-center justify-center font-bold"
                >
                  ✕
                </button>
              </div>
